...
`;
        ...
        // Embedded files (Guaranteed to work)
        zip.file("types.ts", TYPES_CONTENT);
        zip.folder("utils")?.file("gameLogic.ts", GAME_LOGIC_CONTENT);
        zip.folder("utils")?.file("audio.ts", AUDIO_CONTENT);

        // 4. FETCH ONLY MAIN APP (Fallback to current file if fetch fails logic is tricky, so we rely on simple fetch for main entry)
        try {
           ...
           if (response.ok) {
               ...
               if (!text.trim().startsWith('<!DOCTYPE')) {
                   ...
                   // Fallback: If we can't fetch App.tsx (e.g. strict CORS or bundler), alert user
                   alert("Could not automatically fetch App.tsx source. Please copy the code manually if the zip is missing it.");
               }
           }
        } catch (e) {
           console.error("Failed to fetch App.tsx", e);
        }
