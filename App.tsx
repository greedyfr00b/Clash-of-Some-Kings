...
export const App: React.FC = () => {
  ...
  const handleDownloadCode = async () => {
     try {
        ...
        // 1. HELPER FILE CONTENTS (Embedded to prevent fetch errors)
        const TYPES_CONTENT = `
export enum Suit {
  HEARTS = 'HEARTS',
  DIAMONDS = 'DIAMONDS',
...
export interface Player { id: number; name: string; hand: Card[]; isBot: boolean; }

export interface UserProfile {
  name: string; xp: number; level: number; wins: number; gamesPlayed: number;
  googleId?: string; email?: string; avatarUrl?: string;
}

export enum PileSide { LEFT = 'LEFT', RIGHT = 'RIGHT' }
export type GameStatus = 'SETUP' | 'PLAYING' | 'GAME_OVER' | 'TUTORIAL';

export interface GameLogEntry {
  id: string; timestamp: number; playerId: number; playerName: string;
  type: 'PLAY' | 'DRAW' | 'PASS' | 'GAME_OVER'; description: string;
}

export interface ChatMessage {
  id: string; playerId: number; playerName: string; text: string; timestamp: number;
}

export interface GameState {
  deck: Card[]; players: Player[]; currentPlayerIndex: number;
  leftPile: Card[]; rightPile: Card[];
  leftPileFinished: boolean; rightPileFinished: boolean;
  cardsPlayedThisTurn: Card[]; lastPileSidePlayed: PileSide | null;
  status: GameStatus; winner: Player | null; message: string;
  history: GameLogEntry[]; chat: ChatMessage[];
}
...
export interface NetworkAction { type: NetworkActionType; payload?: any; }
`;
...
`;
        ...
        // 2. CONFIG FILES
        // CRITICAL FIX: Moved ALL devDependencies into 'dependencies' to ensure Vercel installs them.
        const packageJson = `
{
  "name": "clash-of-kings",
  "private": true,
...
export default defineConfig({
  plugins: [react()],
  server: {
    host: '0.0.0.0', // Helps with Replit
    hmr: {
      clientPort: 443
    }
  }
...
`;
        ...
        // Critical: vercel.json for correct routing
        const vercelJson = `
{
  "rewrites": [
...
`;
        ...
        // Critical: netlify.toml for Netlify deployments
        const netlifyToml = `
[build]
...
You have 3 easy options to host this game.

### Option 1: Replit (Easiest to see it working)
1. Go to [Replit.com](https://replit.com).
2. Click **Create Repl** -> **Import from GitHub** (or just create a blank **React TypeScript** repl).
3. Upload all these files into the Repl.
4. Click **Run**.
5. Your game will be live at a URL like \`https://clash.username.replit.dev\`.

### Option 2: Vercel (Recommended for Speed)
1. Upload these files to **GitHub**.
2. Go to [Vercel.com](https://vercel.com).
3. Import the repo and click **Deploy**.
4. (Note: All build tools are now in 'dependencies' to prevent 'command not found' errors).

### Option 3: Netlify (Via GitHub)
1. Upload to GitHub.
2. Go to Netlify.com -> **Add New Site** -> **Import an existing project**.
3. Select your repo. Netlify will auto-detect settings.

## ⚠️ CRITICAL: Google Login Setup
No matter which option you choose, Google Login will NOT work until you do this:

1. Go to [Google Cloud Console](https://console.cloud.google.com/apis/credentials).
2. Edit your **OAuth 2.0 Client ID**.
3. Add your new website URL (e.g., \`https://clash.username.replit.dev\`) to **Authorized JavaScript origins**.
4. Save. (Wait 5-10 mins).
`;

        // 3. BUILD ZIP - Use robust paths
        ...
        // Embedded files (Use string paths directly)
        zip.file("types.ts", TYPES_CONTENT);
        zip.file("utils/gameLogic.ts", GAME_LOGIC_CONTENT);
        zip.file("utils/audio.ts", AUDIO_CONTENT);

        // 4. FETCH ONLY MAIN APP 
        try {
           // We try to fetch the raw source code of App.tsx
           const response = await fetch(`./App.tsx`);
           if (response.ok) {
               const text = await response.text();
               // Basic check to ensure we didn't get HTML back
               if (!text.trim().startsWith('<!DOCTYPE')) {
                   zip.file("App.tsx", text);
               } else {
                   throw new Error("Received HTML instead of source");
               }
           } else {
              throw new Error("Fetch failed");
           }
        } catch (e) {
           console.error("Failed to fetch App.tsx, attempting to reconstruct...", e);
           // If fetching fails, we alert the user. 
           // In a real environment we might embed the current file content via a build step,
           // but here we are dynamic. 
           alert("Warning: Could not automatically fetch 'App.tsx' source code due to browser restrictions. The zip file will be missing the main game logic. Please manually copy the code from 'App.tsx' into the downloaded folder.");
           return;
        }
